// =======================================================
// GHL EDC MODULE - USB SERIAL
// Protocol: GHL Systems POS Integration Spec v1.0.17
// Device  : PAX A920 + PaymentDirect UOB / Verifone Vx / Engage
// Baud    : 9600, 8N1
// =======================================================

const { SerialPort } = require('serialport');

// -------------------------------------------------------
// CONSTANTS
// -------------------------------------------------------
const STX = 0x02;
const ETX = 0x03;

const COMMANDS = {
  SALE:       '020',
  VOID:       '022',
  REFUND:     '026',
  SETTLEMENT: '050',
};

const ERROR_CODES = {
  '00': 'Successful',
  'BT': 'Transaction timed out from Bank',
  'CT': 'Transaction timed out / cancelled on Terminal',
  'CD': 'Invalid Check Digit',
};

const CARD_TYPES = {
  '04': 'VISA',
  '05': 'MasterCard',
  '06': 'Diners',
  '07': 'Amex',
  '08': 'MyDebit',
  '09': 'JCB',
  '10': 'UnionPay',
  '11': 'eWallet',
};

// -------------------------------------------------------
// HELPER: Calculate Check Digit (Section 5 of GHL spec)
// - Concat all fields between STX and Check Digit field
// - Break into 8-byte blocks, pad last block with 0xFF
// - XOR all blocks = 8-byte raw Buffer
// -------------------------------------------------------
function calcCheckDigit(data) {
  const buf    = Buffer.from(data, 'ascii');
  const padded = Math.ceil(buf.length / 8) * 8;
  const padBuf = Buffer.alloc(padded, 0xFF);
  buf.copy(padBuf);

  let result = Buffer.alloc(8, 0x00);
  for (let i = 0; i < padBuf.length; i += 8) {
    const block = padBuf.slice(i, i + 8);
    for (let j = 0; j < 8; j++) result[j] ^= block[j];
  }
  return result; // raw 8-byte Buffer
}

// -------------------------------------------------------
// HELPER: Format cashier number
// ANS4 - right-justified, space-padded
// e.g. "1" -> "   1", "12" -> "  12", "1234" -> "1234"
// -------------------------------------------------------
function formatCashier(cashier) {
  return String(cashier || '1').padStart(4, ' ').slice(-4);
}

// -------------------------------------------------------
// HELPER: Build GHL request packet
// Format: STX | CMD(3) | Amount(12) | Invoice(6) | Cashier(4) | CheckDigit(8 raw) | ETX
// Total  : 1 + 3 + 12 + 6 + 4 + 8 + 1 = 35 bytes
// -------------------------------------------------------
function buildPacket(command, amount = 0, invoiceNumber = '000000', cashierNumber = '1') {
  // Amount: N12 zero-padded, already in cents (e.g. RM1.00 = 100)
  const amountStr  = String(parseInt(amount) || 0).padStart(12, '0').slice(-12);

  // Invoice: N6 zero-padded (only used for Void, others pad with 0s)
  const invoiceStr = String(invoiceNumber || '0').padStart(6, '0').slice(-6);

  // Cashier: ANS4 right-justified, space-padded
  const cashierStr = formatCashier(cashierNumber);

  const dataForCheck = command + amountStr + invoiceStr + cashierStr;
  const checkDigit   = calcCheckDigit(dataForCheck); // raw 8-byte Buffer

  const packet = Buffer.concat([
    Buffer.from([STX]),
    Buffer.from(dataForCheck, 'ascii'),
    checkDigit,
    Buffer.from([ETX]),
  ]);

  return packet;
}

// -------------------------------------------------------
// HELPER: Parse GHL response packet (Terminal to POS)
// Format: STX | CMD(3) | ErrCode(2) | CardNum(22) | Expiry(4) |
//         CardType(2) | ApprovalCode(8) | GrossAmt(12) | NetAmt(12) |
//         TraceNum(6) | InvoiceNum(6) | Cashier(4) | CardName(15) |
//         TerminalID(8) | MerchantID(15) | BatchNum(6) | CheckDigit(8) | ETX
// -------------------------------------------------------
function parseResponse(buf) {
  try {
    const str    = buf.toString('ascii');
    const stxIdx = str.indexOf(String.fromCharCode(STX));
    const etxIdx = str.lastIndexOf(String.fromCharCode(ETX));

    if (stxIdx === -1 || etxIdx === -1)
      return { success: false, error: 'Invalid packet: missing STX/ETX' };

    const body  = str.substring(stxIdx + 1, etxIdx);
    let offset  = 0;

    const command       = body.substring(offset, offset += 3);
    const errorCode     = body.substring(offset, offset += 2);
    const cardNumberRaw = body.substring(offset, offset += 22);
    const expiryDate    = body.substring(offset, offset += 4);
    const cardTypeCode  = body.substring(offset, offset += 2);
    const approvalCode  = body.substring(offset, offset += 8).trim();
    const grossAmount   = body.substring(offset, offset += 12);
    const netAmount     = body.substring(offset, offset += 12);
    const traceNumber   = body.substring(offset, offset += 6);
    const invoiceNumber = body.substring(offset, offset += 6);
    const cashierNumber = body.substring(offset, offset += 4).trim();
    const cardName      = body.substring(offset, offset += 15).trim();
    const terminalId    = body.substring(offset, offset += 8).trim();
    const merchantId    = body.substring(offset, offset += 15).trim();
    const batchNumber   = body.substring(offset, offset += 6).trim();

    // Card number: first 2 bytes = length, rest = masked card number
    const cardLength = parseInt(cardNumberRaw.substring(0, 2)) || 0;
    const cardNumber = cardNumberRaw.substring(2, 2 + cardLength);

    return {
      success:        errorCode === '00',
      command,
      errorCode,
      errorMessage:   ERROR_CODES[errorCode] || `Error ${errorCode}`,
      cardNumber,
      expiryDate,
      cardTypeCode,
      cardType:       CARD_TYPES[cardTypeCode] || `Unknown (${cardTypeCode})`,
      cardName,
      approvalCode,
      grossAmount:    parseInt(grossAmount) || 0,
      netAmount:      parseInt(netAmount)   || 0,
      traceNumber:    traceNumber.trim(),
      invoiceNumber:  invoiceNumber.trim(),
      cashierNumber,
      terminalId,
      merchantId,
      batchNumber,
    };
  } catch (err) {
    return { success: false, error: `Parse error: ${err.message}` };
  }
}

// -------------------------------------------------------
// GHL EDC CLASS - USB SERIAL
// -------------------------------------------------------
class GhlEDC {
  constructor(options = {}) {
    this.portPath    = options.portPath || 'COM6';
    this.baudRate    = options.baudRate || 9600;
    this.timeout     = options.timeout  || 60000; // 60s - customer needs time to swipe
    this.cashier     = options.cashier  || '1';    // default cashier number
    this.port        = null;
    this.isConnected = false;
  }

  // -------------------------------------------------------
  // Open serial port
  // -------------------------------------------------------
  connect() {
    return new Promise((resolve, reject) => {
      this.port = new SerialPort({
        path:     this.portPath,
        baudRate: this.baudRate,
        dataBits: 8,
        stopBits: 1,
        parity:   'none',
        autoOpen: false,
      });

      this.port.open((err) => {
        if (err) return reject(new Error(`Failed to open ${this.portPath}: ${err.message}`));
        this.isConnected = true;
        console.log(`[GHL] Connected: ${this.portPath} @ ${this.baudRate} baud`);
        resolve();
      });

      this.port.on('error', (err) => {
        console.error(`[GHL] Serial error: ${err.message}`);
        this.isConnected = false;
      });

      this.port.on('close', () => {
        this.isConnected = false;
      });
    });
  }

  // -------------------------------------------------------
  // Close serial port
  // -------------------------------------------------------
  disconnect() {
    return new Promise((resolve) => {
      if (this.port && this.port.isOpen) {
        this.port.close(() => {
          this.isConnected = false;
          resolve();
        });
      } else {
        resolve();
      }
    });
  }

  // -------------------------------------------------------
  // Send packet and wait for complete response (ends with ETX)
  // -------------------------------------------------------
  sendAndReceive(packet) {
    return new Promise((resolve, reject) => {
      let responseBuffer = Buffer.alloc(0);
      let timer = null;

      const cleanup = () => {
        if (timer) clearTimeout(timer);
        this.port.removeAllListeners('data');
      };

      timer = setTimeout(() => {
        cleanup();
        reject(new Error('EDC transaction timeout - card was not swiped in time'));
      }, this.timeout);

      this.port.on('data', (chunk) => {
        responseBuffer = Buffer.concat([responseBuffer, chunk]);
        if (responseBuffer.indexOf(ETX) !== -1) {
          cleanup();
          console.log('[GHL USB] ===== RAW RESPONSE RECEIVED =====');
          console.log('[GHL USB] HEX   :', responseBuffer.toString('hex'));
          console.log('[GHL USB] BYTES :', [...responseBuffer].map(b => b.toString(16).padStart(2, '0').toUpperCase()).join(' '));
          console.log('[GHL USB] LEN   :', responseBuffer.length, 'bytes');
          console.log('[GHL USB] ==========================================');
          resolve(responseBuffer);
        }
      });

      // Log raw packet being sent
      console.log('[GHL USB] ===== RAW PACKET SENT =====');
      console.log('[GHL USB] HEX   :', packet.toString('hex'));
      console.log('[GHL USB] BYTES :', [...packet].map(b => b.toString(16).padStart(2, '0').toUpperCase()).join(' '));
      console.log('[GHL USB] LEN   :', packet.length, 'bytes');
      console.log('[GHL USB] ====================================');

      this.port.write(packet, (err) => {
        if (err) {
          cleanup();
          reject(new Error(`Write error: ${err.message}`));
        }
      });
    });
  }

  // -------------------------------------------------------
  // SALE - amount in cents (RM1.00 = 100)
  // cashier: optional, overrides constructor default
  // -------------------------------------------------------
  async sale(amount, cashier, options = {}) {
    if (!this.isConnected) await this.connect();
    const cashierNum = cashier || this.cashier;
    const packet = buildPacket(COMMANDS.SALE, amount, '000000', cashierNum);
    console.log(`[GHL] Sale - Amount: ${amount} cents | Cashier: ${formatCashier(cashierNum)}`);
    const result = parseResponse(await this.sendAndReceive(packet));
    console.log(`[GHL] Sale result: ${result.errorMessage} | Approval: ${result.approvalCode}`);
    return result;
  }

  // -------------------------------------------------------
  // VOID - invoiceNumber from original receipt
  // cashier: optional, overrides constructor default
  // -------------------------------------------------------
  async void(invoiceNumber, cashier, options = {}) {
    if (!this.isConnected) await this.connect();
    const cashierNum = cashier || this.cashier;
    const packet = buildPacket(COMMANDS.VOID, 0, invoiceNumber, cashierNum);
    console.log(`[GHL] Void - Invoice: ${invoiceNumber} | Cashier: ${formatCashier(cashierNum)}`);
    const result = parseResponse(await this.sendAndReceive(packet));
    console.log(`[GHL] Void result: ${result.errorMessage}`);
    return result;
  }

  // -------------------------------------------------------
  // REFUND - amount in cents
  // cashier: optional, overrides constructor default
  // -------------------------------------------------------
  async refund(amount, cashier, options = {}) {
    if (!this.isConnected) await this.connect();
    const cashierNum = cashier || this.cashier;
    const packet = buildPacket(COMMANDS.REFUND, amount, '000000', cashierNum);
    console.log(`[GHL] Refund - Amount: ${amount} cents | Cashier: ${formatCashier(cashierNum)}`);
    const result = parseResponse(await this.sendAndReceive(packet));
    console.log(`[GHL] Refund result: ${result.errorMessage}`);
    return result;
  }

  // -------------------------------------------------------
  // SETTLEMENT - batch close
  // cashier: optional, overrides constructor default
  // -------------------------------------------------------
  async settlement(cashier, options = {}) {
    if (!this.isConnected) await this.connect();
    const cashierNum = cashier || this.cashier;
    const packet = buildPacket(COMMANDS.SETTLEMENT, 0, '000000', cashierNum);
    console.log(`[GHL] Settlement | Cashier: ${formatCashier(cashierNum)}`);
    const result = parseResponse(await this.sendAndReceive(packet));
    console.log(`[GHL] Settlement result: ${result.errorMessage}`);
    return result;
  }
}

module.exports = { GhlEDC };
