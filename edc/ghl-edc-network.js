...
class GhlEDCNetwork {
  constructor(options = {}) {
    ...
    this.timeout = options.timeout || 120000; // 120s — customer needs time to swipe
  ...
  sendAndReceive(packet) {
    return new Promise((resolve, reject) => {
      ...
      client.connect(this.port, this.host, () => {
        ...
        console.log('[GHL NET] ===== RAW PACKET SENT =====');
        console.log('[GHL NET] HEX :', packet.toString('hex'));
        console.log('[GHL NET] BYTES:', [...packet].map(b => b.toString(16).padStart(2,'0').toUpperCase()).join(' '));
        console.log('[GHL NET] LEN  :', packet.length, 'bytes');
        console.log('[GHL NET] ==============================');
        client.write(packet);
      });
      ...
      client.on('data', (chunk) => {
        ...
        if (responseBuffer.indexOf(ETX) !== -1) {
          console.log('[GHL NET] ===== RAW RESPONSE RECEIVED =====');
          console.log('[GHL NET] HEX :', responseBuffer.toString('hex'));
          console.log('[GHL NET] BYTES:', [...responseBuffer].map(b => b.toString(16).padStart(2,'0').toUpperCase()).join(' '));
          console.log('[GHL NET] LEN  :', responseBuffer.length, 'bytes');
          console.log('[GHL NET] ======================================');
